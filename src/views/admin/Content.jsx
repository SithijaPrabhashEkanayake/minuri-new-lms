...
export function Content() {
  ...
  const handleUpload = async (e) => {
    ...
    try {
      ...
      const credResponse = await fetch(`/api/video/upload-credentials`, {
        method: 'POST',
        headers: { 
      ...
      saveFormData.append('videoId', videoId);
      if (thumbnail) {
        saveFormData.append('thumbnail', thumbnail);
      }

      const saveResponse = await fetch(`/api/modules/${moduleIdToUpload}/videos`, {
        method: 'POST',
        headers: {
          'Authorization': `Bearer ${user.token}`
        },
