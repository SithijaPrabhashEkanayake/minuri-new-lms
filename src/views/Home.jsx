  ...
  const ModuleCard = ({ m, action = 'Enter LMS' }) => (
    <div className="glass card-pad fade-up">
      ...
      <div className="row between">
        ...
        <button className="btn btn-primary btn-sm" onClick={() => router.push('/lms')}>{action}</button>
      </div>
    </div>
  );
    ...
    <div className="wrap">
      <section className="hero">
        <div className="fade-up">
          ...
          <div className="cta-row" style={{ marginTop: '32px' }}>
            <button className="btn btn-primary" onClick={() => router.push('/lms')}>Start Learning</button>
            <button className="btn btn-glass" onClick={() => router.push('/blog')}>Read the blog</button>
          </div>
      ...
      <section className="section-sm">
        <div className="glass card-pad fade-up" style={{ padding: '46px', display: 'flex', justifyContent: 'space-between', alignItems: 'center', gap: '24px', flexWrap: 'wrap', background: 'var(--grad-hero)' }}>
          ...
          <button className="btn" style={{ background: '#fff', color: 'var(--blue-d)' }} onClick={() => router.push('/lms')}>Enter the LMS →</button>
        </div>
      </section>
    </div>
  );
